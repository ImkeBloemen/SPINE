...
def run_experiment(name, classifier_names, classifier_paths, tt_number, epochs_dataset, classes_mult):
    ...
    def get_layer_output(model, X, layer_index=-2):
        """
    ...
    for classifier_path, clf_name in zip(classifier_paths, classifier_names):
        ...
        if clf_name == "LogisticRegression":
            ...
            clf = Sequential()
            clf.add(tf.keras.layers.InputLayer(input_shape=(X_train.shape[1],)))
        ...
        # Force symbolic graph build (required before Functional slicing!)
        clf.build(input_shape=(None, X_train.shape[1]))
        clf(X_train[:1])

        # Correct penultimate hidden dense = index 4
        last_hidden_index = -2

        print(f"Extracting hidden layer at index {last_hidden_index}...")

        X_train_hidden = get_layer_output(clf, X_train, layer_index=last_hidden_index)
        X_test_hidden  = get_layer_output(clf, X_test,  layer_index=last_hidden_index)

        print("Hidden-layer extraction completed.")
        print("Shapes:", X_train_hidden.shape, X_test_hidden.shape)
        ...
        ssnpgt.fit(X_train_hidden, y_train)
        ...
        X_ssnpgt = ssnpgt.transform(X_train_hidden)
        ...
        confidence_real = clf.predict(X_train_hidden)
        confidence_real = np.max(confidence_real, axis=1)

        X_test_ssnpgt = ssnpgt.transform(X_test_hidden)
        ...
        probs_test = clf.predict(X_test_hidden)
...
classifier_names = ["MLP"]#['underfit_model', 'balanced_model', 'overfit_model']#['MLP']#["LogisticRegression", "MLP"]
classifier_paths = ["MLP.h5"] #['underfit_model', 'balanced_model', 'overfit_model']#['MLP.h5'] #["LogisticRegression.h5", "MLP.h5"]
