...
def run_experiment(name, classifier_names, classifier_paths, tt_number, epochs_dataset, classes_mult):
    ...
    def results_to_png(np_matrix, prob_matrix, grid_size, n_classes, dataset_name, classifier_name, 
        ...
        if test_set_trans is not None:
            ...
            scatter = ax.scatter(test_set_trans[:, 1], test_set_trans[:, 0], c=test_labels_trans, cmap=custom_cmap, vmin=0,
                        vmax=n_classes - 1, s=20, edgecolors='k', label='Adversarial Test Points')
            plt.legend(loc='upper right', fontsize=16, title="Legend")
            ax.axis('off')  # Remove axes
            plt.title("Decision Boundary Mapping with Adversarial Test Points", font='Times New Roman', fontsize=25)
        ...
        if test_set_trans is not None and test_set_subset is not None:
            ...
            ax.scatter(test_set_subset[:, 1], test_set_subset[:, 0], c=test_subset_pred, cmap=custom_cmap, s=35, 
                        vmin=0,
                        vmax=n_classes - 1,    # Size of the test points
                        alpha=1,
            ...
            ax.scatter(test_set_trans[:, 1], test_set_trans[:, 0], c=test_labels_trans,s=35, cmap=custom_cmap,
                vmin=0,
                vmax=n_classes - 1,  
                alpha=1,
                edgecolor='black',  # Add a black edge color
    ...
    X_test_trans = np.load(os.path.join(data_dir, f'X_test_adv.npy'))
    y_test_trans = np.load(os.path.join(data_dir,f'y_test_adv.npy'))
    ...
    for classifier_path, clf_name in zip(classifier_paths, classifier_names):
        ...
        # probs_test = clf.predict(X_test_trans)
        # labels_test = np.argmax(probs_test, axis=1)
        ...
        points_2d, hd_recon, orig_probs_adv, recon_probs_adv, pixel_labels = evaluate_boundary_mapping_batch_sdbm(
            ssnp_model=ssnpgt,
            clf=clf,
        ...
        recon_accuracy_adv = np.mean(np.argmax(orig_probs_adv, axis=1) == np.argmax(recon_probs_adv, axis=1))
        ...
        results = {
                ...
                "accuracy_adv": recon_accuracy_adv,
...
name = 'winequality-white_adv'
classifier_names =  ['MLP']#["LogisticRegression", "MLP"]
classifier_paths = ['MLP'] #["LogisticRegression.h5", "MLP.h5"]
