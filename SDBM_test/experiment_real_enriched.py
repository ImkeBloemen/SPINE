...
def run_experiment(name, classifier_names, classifier_paths, tt_number, epochs_dataset, classes_mult):
    ...
    def results_to_png(np_matrix, prob_matrix, grid_size, n_classes, dataset_name, classifier_name, output_dir =output_dir, real_points=None, real_labels=None, real_confidence=None, indexes_in_range=None, indexes_confidences=None,
                       enriched_points=None, enriched_confidence=None, enriched_prediction=None, max_value_hsv=None, suffix=None, normalized_new_point=None, test_set=None, test_labels=None):
        ...
        if normalized_new_point is not None:
            ...
            ax.axis('off')  # Remove axes
            plt.savefig(os.path.join(output_dir, f"{classifier_name}_{grid_size}x{grid_size}_{dataset_name}_vanilla_new_point{suffix}.png"), bbox_inches='tight', pad_inches=0.1)
            plt.close(fig)
        ...
        # Vanilla plot with enriched points
        if enriched_points is not None and enriched_confidence is not None:
            fig, ax = plt.subplots(figsize=(8, 8), dpi=300)
            ...
            # Create the scatter object (the mappable)
            scatter = ax.scatter(
                enriched_points[:, 1],
                enriched_points[:, 0],
                c=enriched_confidence,
                cmap=cmap,
                norm=norm,
                s=5,
                alpha=1,
                edgecolors='none',
                marker='o',
                label='Enriched Points'
            )

            # Now attach the colorbar to the scatter object, telling it which Axes to steal space from
            cbar = fig.colorbar(scatter, ax=ax, label="Confidence", shrink=0.6, aspect=15)
            
            plt.legend(loc='upper right', fontsize=16, title="Legend")
            ax.axis('off')  # Remove axes
            plt.savefig(os.path.join(output_dir, f"{classifier_name}_vanilla_enriched_points{suffix}.png"),
                        bbox_inches='tight', pad_inches=0)
            plt.close(fig)

        if enriched_points is not None and enriched_prediction is not None:
            fig, ax = plt.subplots(figsize=(8, 8), dpi=300)
            ...
            # Now attach the colorbar to the scatter object, telling it which Axes to steal space from
            # cbar = fig.colorbar(scatter, ax=ax, label="Prediction", shrink=0.6, aspect=15)
            
            plt.legend(loc='upper right', fontsize=16, title="Legend")
            ax.axis('off')  # Remove axes
            plt.savefig(os.path.join(output_dir, f"{classifier_name}_vanilla_enriched_points_pred{suffix}.png"),
                        bbox_inches='tight', pad_inches=0)
            plt.close(fig)
        ...
        if real_points is not None and real_labels is not None:
            fig, ax = plt.subplots(figsize=(8, 8), dpi=300)
            ax.imshow(data_vanilla, origin='lower')
            scatter = ax.scatter(real_points[:, 1], real_points[:, 0], c=real_labels, cmap='tab20', s=20, edgecolors='k', label='Train Points')
            ax.axis('off')  # Remove axes
            plt.legend(loc='upper right', fontsize=16, title="Legend")
    ...
    X_enriched = np.load(os.path.join(data_dir, 'enriched', f'X_enriched.npy'))
    y_enriched = np.load(os.path.join(data_dir, 'enriched', f'y_enriched.npy'))
    ...
    for classifier_path, clf_name in zip(classifier_paths, classifier_names):
        ...
        enriched_ssnpgt = ssnpgt.transform(X_enriched)
        ...
        enriched_pred = clf.predict(X_enriched)
        enriched_confidence = np.max(enriched_pred, axis=1)
        enriched_prediction = np.argmax(enriched_pred, axis=1)
        ...
        normalized_enriched = scaler_2D.transform(enriched_ssnpgt)
        normalized_enriched = normalized_enriched.astype('float32')
        normalized_enriched *= (grid_size - 1)
        normalized_enriched = normalized_enriched.astype(int)
        normalized_enriched = np.clip(normalized_enriched, 0, grid_size - 1)

        print("normalized enriched", normalized_enriched)
        print("enriched confidence", enriched_confidence)

        # new_point = pd.read_csv('input_data/breast_cancer/x_new.csv').values[0]
        new_point = np.array([0.49074731, 0.45147109, 0.46444613, 0.33521545, 0.29334469, 0.13348874,
        ...
        # scaler_filename = os.path.join(data_dir, 'scaler.pkl')
        # with open(scaler_filename, 'rb') as scaler_file:
        #     scaler = pickle.load(scaler_file)
        # new_point_scaled = scaler.transform(new_point.reshape(1, -1))
        new_point_2D = ssnpgt.transform(new_point.reshape(1, -1))
        normalized_new_point = scaler_2D.transform(new_point_2D).astype('float32') * (grid_size - 1)
        ...
        results_to_png(np_matrix=img_grid, 
                    ...
                    enriched_points=normalized_enriched,
