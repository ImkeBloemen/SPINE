...
class PublicData(_BaseData):
    ...
    #     training_labels = self.data_df[self.outcome_name].values
    #     encoded_data = self.encoder(self.data_df)
    #     self.class_prototypes = {}
    #     unique_labels = np.unique(training_labels)
    #     for label in unique_labels:
    #         # Get indices of instances belonging to the current class
    #         class_indices = np.where(training_labels == label)[0]
    #         # Compute the prototype as the mean of encoded instances of this class
    #         self.class_prototypes[label] = tf.reduce_mean(tf.gather(encoded_data, class_indices), axis=0)
