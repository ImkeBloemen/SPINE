...
class Dice(ExplainerBase):
    ...
    def decide_implementation_type(self, data_interface, model_interface, method, **kwargs):
        ...
        self.__init__(data_interface, model_interface, **kwargs) #Initiaize the Explainerbase
...
def decide(model_interface, method):
    ...
    if method == SamplingStrategy.Random:
        ...
        from VAE_DBS.models.DiCE.dice_ml.explainer_interfaces.dice_random import DiceRandom
        return DiceRandom
    elif method == SamplingStrategy.Genetic:
        from VAE_DBS.models.DiCE.dice_ml.explainer_interfaces.dice_genetic import DiceGenetic
        return DiceGenetic
    elif method == SamplingStrategy.KdTree:
        from VAE_DBS.models.DiCE.dice_ml.explainer_interfaces.dice_KD import DiceKD
        return DiceKD
        ...
        if model_interface.backend == BackEndTypes.Tensorflow1:
            ...
            from VAE_DBS.models.DiCE.dice_ml.explainer_interfaces.dice_tensorflow1 import \
                DiceTensorFlow1
            return DiceTensorFlow1

        elif model_interface.backend == BackEndTypes.Tensorflow2: #We are using this one
            # pretrained Keras Sequential model with Tensorflow 2.x backend
            from VAE_DBS.models.DiCE.dice_ml.explainer_interfaces.dice_tensorflow2 import \
                DiceTensorFlow2
            return DiceTensorFlow2

        elif model_interface.backend == BackEndTypes.Pytorch:
            # PyTorch backend
            from VAE_DBS.models.DiCE.dice_ml.explainer_interfaces.dice_pytorch import DicePyTorch
            return DicePyTorch
        else:
            raise UserConfigValidationException(
