...
input_data_dir = '../../../../data/experiment_input/reduced_mnist_data_gen'
input_model_dir = '../input_classifiers/evaluation/reduced_mnist_data_gen'
...
X_train = pd.read_csv('../../../../data/reduced_mnist_data_train.csv')
...
X_test = pd.read_csv('../../../../data/reduced_mnist_data_test.csv')
y_test = X_test['label'].values
X_test = X_test.drop('label', axis=1).values

X_test_trans = pd.read_csv('../../../../data/reduced_mnist_data_test_trans.csv')
...
X_test_control = pd.read_csv('../../../../data/reduced_mnist_data_test_control.csv')
y_test_control = X_test_control['label'].values
X_test_control = X_test_control.drop('label', axis=1).values

X_test = scaler.transform(X_test)
X_test_control = scaler.transform(X_test_control)

y_train = y_train.astype(int)
y_test = y_test.astype(int)
y_test_trans = y_test_trans.astype(int)
y_test_control = y_test_control.astype(int)
...
y_test_total_one_hot = to_categorical(y_test_control, num_classes=n_classes)
...
np.save(os.path.join(input_data_dir_tt, "y_test_trans.npy"), y_test_trans)
np.save(os.path.join(input_data_dir_tt, "X_test_control.npy"), X_test_control)
np.save(os.path.join(input_data_dir_tt, "y_test_control.npy"), y_test_control)
...
for classifier_path, clf_name in zip(classifier_paths, classifier_names):
    ...
    if clf_name == "LogisticRegression":
        ...
        loss, accuracy = clf.evaluate(X_test, y_test_one_hot, verbose=0)
        loss_trans, accuracy_trans = clf.evaluate(X_test_trans, y_test_trans_one_hot, verbose=0)
        loss_total, accuracy_total = clf.evaluate(X_test, y_test_one_hot, verbose=0)
