...
class PredictionMap:
    def __init__(self, grid_size, original_data, intermediate_gradient_points, counterfactuals, 
                 number_of_neighbors, model_for_predictions, scaler_path_2D: str = None, projection_method='umap', projection_name=None,
                 all_gradients=None, intermediate_predictions=None, original_predictions=None, counterfactual_predictions=None,
                 outcome_name='target', n_classes=2, version=1, comparison=False, dataset_name=None, gradients=False, hidden_layer=False, filter_jd=False):
        ...
        if isinstance(intermediate_gradient_points, pd.DataFrame):
            self.intermediate_gradient_points = intermediate_gradient_points.drop(columns=[outcome_name]) if outcome_name in intermediate_gradient_points.columns else intermediate_gradient_points
        ...
        if isinstance(counterfactuals, pd.DataFrame):
            self.counterfactuals = counterfactuals.drop(columns=[outcome_name]) if outcome_name in counterfactuals.columns else counterfactuals
            self.counterfactuals = self.counterfactuals.drop(columns='class') if 'class' in self.counterfactuals.columns else self.counterfactuals
        ...
        if isinstance(all_gradients, pd.DataFrame):
            self.all_gradients = all_gradients.drop(columns=[outcome_name]) if outcome_name in all_gradients.columns else all_gradients
        ...
        self.intermediate_gradient_points_stacked = np.concatenate((self.intermediate_gradient_points, self.counterfactuals))
        self.intermediate_predictions_stacked = np.vstack((reshaped_intermediate_predictions, reshaped_counterfactual_predictions))

        if self.hidden_layer:
            self.original_data_for_predictions = self.original_data.copy()
            self.intermediate_gradient_points_stacked_for_predictions = self.intermediate_gradient_points_stacked.copy()
            self.original_data = self.get_layer_output(self.model_for_predictions, self.original_data)
            self.intermediate_gradient_points_stacked = self.get_layer_output(self.model_for_predictions, self.intermediate_gradient_points_stacked)
    ...
    def to_grid(self, points_2D, grid_size=300):
        ...
        # 1. Shift so min corner is at (0, 0)
    ...
    def fit_points_2D(self, path=None, input_path=None):
        ...
        #Project intermediate gradient points and optionally their gradient sizes
        if self.projection_method == 'umap':
            self.intermediate_points_2D = self.umap_reducer.transform(self.intermediate_gradient_points_stacked)
            if self.gradients:
                self.intermediate_points_2D_gradients = self.umap_reducer.transform(self.intermediate_gradient_points)
            
            self.normalized_intermediate = self.scaler_2D.transform(self.intermediate_points_2D) * (self.grid_size - 1)
            self.normalized_intermediate = np.clip(self.normalized_intermediate, 0, self.grid_size - 1).astype(int)

            if self.gradients:
                self.normalized_intermediate_gradients = self.scaler_2D.transform(self.intermediate_points_2D_gradients).astype('float32') * (self.grid_size - 1)
                self.normalized_intermediate_gradients = np.clip(self.normalized_intermediate_gradients, 0, self.grid_size - 1).astype(int)

        elif self.projection_method == 'ssnp':
            self.intermediate_points_2D = self.ssnp_reducer.transform(self.intermediate_gradient_points_stacked)
            if self.gradients:
                self.intermediate_points_2D_gradients = self.ssnp_reducer.transform(self.intermediate_gradient_points)

            self.normalized_intermediate = self.scaler_2D.transform(self.intermediate_points_2D).astype("float32") * (self.grid_size - 1)
            self.normalized_intermediate = np.clip(self.normalized_intermediate, 0, self.grid_size - 1).astype(int)
            ...
            self.intermediate_points_2D = self.lamp_reducer.transform(self.intermediate_gradient_points_stacked)
            if self.gradients:
                self.intermediate_points_2D_gradients = self.lamp_reducer.transform(self.intermediate_gradient_points)
            
            self.normalized_intermediate = self.to_grid(self.intermediate_points_2D, grid_size=self.grid_size)
            if self.gradients:
                self.normalized_intermediate_gradients = self.to_grid(self.intermediate_points_2D_gradients)
                

        elif self.projection_method == 'PCA':
            self.intermediate_points_2D = self.pca_reducer.transform(self.intermediate_gradient_points_stacked)
            if self.gradients:
                self.intermediate_points_2D_gradients = self.pca_reducer.transform(self.intermediate_gradient_points)
        ...
        self.combined_normalized_2D = np.vstack((self.normalized_original, self.normalized_intermediate))

        if self.filter_jd == True:
            jaccard_values = self.compute_jaccard_and_filter_entire_set(k=15, remove_ratio=0.15)
        ...
        plt.scatter(self.normalized_intermediate[:, 0], self.normalized_intermediate[:, 1], c='red', marker='x', label='Intermediate Gradient Points')
    ...
    def compute_jaccard_and_filter_entire_set(self, k=15, remove_ratio=0.15):
        """
        1) Build a single combined nD array (original_data + intermediate_gradient_points_stacked)
        whose length matches self.combined_normalized_2D.
        2) Compute JD_k(i) for each point i by comparing k-nearest neighbors in nD vs 2D.
        3) Remove the bottom 'remove_ratio' (e.g. 0.15 == 15%) from both
        the nD data and 2D data, so that only the top JD points remain.
        """

        # -------------------------------------------------
        # 1) Combine the nD data to match self.combined_normalized_2D
        # -------------------------------------------------
        ...
        # -------------------------------------------------
        # 4) Compute Jaccard for every point i in the combined set
        # -------------------------------------------------
        ...
        # -------------------------------------------------
        # 5) Sort by JD value and remove bottom 15%
        # -------------------------------------------------
        cutoff_count = int(remove_ratio * len(nD_data))
        sorted_idx = np.argsort(jaccard_vals)  # ascending order of JD
    ...
    def plot_data_with_predictions(self, grid_lines=False, X_test=None, path=None):
        ...
        # Overlay intermediate gradient points
        scatter2 = ax.scatter(
        ...
        if X_test is not None:
            ...
            if self.n_classes == 2:
                ...
                if y_pred_proba.ndim > 1 and y_pred_proba.shape[1] > 1:
                    y_pred_proba = y_pred_proba[:, 1] # Consider the positive class
                y_pred = (y_pred_proba > 0.5).astype(int)
            else:
                # Multi-class => take arg
                y_pred = np.argmax(y_pred_proba, axis=1)
            # Step 1: Project X_test to 2D - use copies to avoid unnecessary computation
            if self.projection_method == 'umap':
                ...
                X_test_2D = self.X_test_2D.copy()
                scaled_2D = self.scaled_2D.copy()
            else: #In case of two feature scatterplot
                X_test_2D = self.X_test_2D.copy()
                scaled_2D = self.scaled_2D.copy()
    ...
    def fit_grid_multilateration(self, path=None):
        ...
        if self.hidden_layer:
            combined_hd_data = np.vstack((self.original_data_for_predictions, self.intermediate_gradient_points_stacked_for_predictions))
        else:
            # Combine high-dimensional data
            combined_hd_data = np.vstack((self.original_data, self.intermediate_gradient_points_stacked))
        ...
        self.combined_2D_data = np.vstack((self.original_data_2D, self.intermediate_points_2D))
        ...
        for start_idx in range(0, total_grid_points, batch_size):
            ...
            #Use Gaussian weights for weighting the neighbouring points
    ...
    def evaluate_mapping_testset(self, X_test):
        ...
        # Use copies to prevent unnecessary computation
        preds_hd = self.preds_hd.copy()

        if self.n_classes == 2:
            # Binary classification => threshold
            if preds_hd.ndim > 1 and preds_hd.shape[1] > 1:
                preds_hd = preds_hd[:, 1] #Use the positive class when binary
    ...
    def plot_test_points_on_mapping(self, X_test, path, y_test=None, X_test_trans=None, y_test_trans=None, version='', X_test_subset=None, y_test_subset=None, adversarial=False, generalize=False):
        """
        ...
        # Print test points with prediction probability between 0.40 and 0.60
        # for i, prob in enumerate(y_pred_proba):
        #     p = prob[1]
        #     if 0.40 <= p <= 0.60:
                #print(f"Test point {i} has a prediction probability of {prob} and values: {X_test[i]}")
        if self.hidden_layer:
        ...
        if self.projection_method == 'umap':
            ...
            self.scaled_2D = self.to_grid(X_test_2D, grid_size=self.grid_size)
            scaled_2D = self.scaled_2D.copy()
        else: # In case of feature-to-feature scatterplot without projection
        ...
        sns.set_theme(style="whitegrid", font="Times New Roman", font_scale=1.2)
        # Step 5: Plot the grid image with test points overlaid
        plt.figure(figsize=(10, 10))
    ...
    def plot_gradient(self, path):
        plot_gradient_grid(self.image, self.version, self.normalized_intermediate_gradients, self.grid_points_2D, self.grid_size, self.all_gradients, self.scaler_2D, path)
