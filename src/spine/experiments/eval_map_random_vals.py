...
def generate_random_feature_combinations(X_train, n_new_samples=2700, random_state=None):
    """
    Generates new data samples by taking each feature value from a random row of X_train.
    
    Parameters
    ----------
    X_train : np.ndarray
        Original training data of shape (n_samples, n_features).
    n_new_samples : int, optional
        Number of new samples to generate (default: 300).
    random_state : int or None, optional
        Random seed for reproducibility (default: None).
    
    Returns
    -------
    X_combinations : np.ndarray
        New data array of shape (n_new_samples, n_features), where each
        feature in a new sample is taken from a random row in X_train.
    """
    rng = np.random.default_rng(seed=random_state)
    
    n_samples, n_features = X_train.shape
    
    # Prepare empty array for new samples
    X_combinations = np.zeros((n_new_samples, n_features))
    
    # For each new sample
    for i in range(n_new_samples):
        # For each feature in that sample
        for j in range(n_features):
            # Pick a random row index
            row_idx = rng.integers(0, n_samples)
            # Take that feature value
            X_combinations[i, j] = X_train[row_idx, j]
...
def run_experiment(model_path, tt_number, projection_method, input_path, dataset_name, model_names, outcome_name, output_path_train_test, test_input_path, grid_size, n_classes):
    ...
    for model_name in model_names:
        # Load input data
        intermediate_points = generate_random_feature_combinations(np.load(os.path.join(test_input_path, f'X_train.npy')))
        ...
        intermediate_y_pred = model_for_predictions.predict(intermediate_points)
...
if __name__ == '__main__':
    ...
    for tt_number in tt_numbers:
        run_experiment(
            ...
            tt_number=tt_number,
            projection_method=projection_method,
            input_path=f'../../../results/experiment_output/{projection_method}/reduced_mnist_data/train_test_{tt_number}',
            dataset_name='reduced_mnist_data',
            model_names=['MLP'],
            outcome_name='label',
            output_path_train_test=f'../../../results/experiment_output/{projection_method}/reduced_mnist_data_random_vals/train_test_{tt_number}',
            test_input_path=f'../../../data/experiment_input/reduced_mnist_data/train_test_{tt_number}',
            grid_size=300,
            n_classes=10
        )
